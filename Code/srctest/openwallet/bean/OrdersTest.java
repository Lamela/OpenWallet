...
public class OrdersTest {
  ...
  public void avantTest(){
    ...
    this.address = new Address(myUser,"Jean",
  ...
  public void testAdd() {
    ...
    assertEquals(this.assertOrdersEquals(this.orders, orders2), true);
  }
  ...
  public void testUpdate(){
    ...
    assertEquals(orders2.getCreate_date_order().equals(new Date(0).valueOf("2018-01-18")), true);
  }
  ...
  public void testDelete(){
    ...
    id_orders2 = ordersDAO.add(orders2);
    ordersDAO.delete(id_orders2);

    assertEquals(ordersDAO.get(id_orders2), null);
  }
  ...
  private boolean assertOrdersEquals(Orders orders1,Orders orders2) {
    ...
    if(orders1.getId_order()!=orders2.getId_order()) {
      ...
      if(!orders1.getUser().getId_user().equals(orders2.getUser().getId_user())) {
        ...
        if(!orders1.getStatus().equals(orders2.getStatus())) {
          ...
          if(!orders1.getTotal_price().equals(orders2.getTotal_price())) {
            System.out.println("TestOrders Error: total_price not equals !");
          } else {
            if(!orders1.getInvoice_address().getId_address().equals(orders2.getInvoice_address().getId_address())) {
              System.out.println("TestOrders Error: invoice_address not equals !");
            } else {
              if(!orders1.getDelivery_address().getId_address().equals(orders2.getDelivery_address().getId_address())) {
