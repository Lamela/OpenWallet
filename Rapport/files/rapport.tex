...
\title{TW2 : Rapport de Projet - Site de Vente}


\author{Groupe 6 : Xu Jitao, Lu Chenxin, Zhu Zhenghui, Osmani Wael, Rasendrasoa
Sandratra}
...
\begin{figure}[H]
\includegraphics[angle=-90,scale=0.28]{/home/rasendrasoa/OpenWallet/Conception/diagramme_nav}
...
\begin{figure}[H]
\includegraphics[angle=-90,scale=0.28]{/home/rasendrasoa/OpenWallet/Conception/diagramme_classe}
...
\begin{figure}[H]
\includegraphics[scale=0.5]{collé1}
...
L'ensemble des tests a été réalisé sur le Modèle, les tests sur le
Controleur et la Vue étant optionnels et demandant trop de temps à
implémenter. Les fichiers suivants ont donc été testés via JUnit : 
\begin{enumerate}
\item Address.java
\item Category.java
\item Comment.java
...
\item User.java
\end{enumerate}
L'exécution des tests se fait via le script \textasciitilde{}/OpenWallet/Code/compile.sh
en exécutant les commandes suivantes dans le terminal :
\begin{enumerate}
\item bash -x compile.sh compile : compilation des fichiers de test
\item bash -x compile.sh exec : exécute la suite de test SuiteAll
\end{enumerate}
...
afin de compiler l'ensemble du code source et créer l'archive .jar.
L'archive est alors disponible sous le nom \textasciitilde{}/OpenWallet/Code/web/Openwallet.war.
Il suffit ensuite de copier cette archive dans le répoertoire qui
y est dédié dans serveur pour le déploiement.
